...
UUID 0xFE55 の Service Data に構造化 JSON を格納するコミュニティ放送プロトコルです。

> ❌ **送信（`beacon_start` / `beacon_stop`）は v8.9.6 で非対応**。`beacon_start()` は `nil, "not supported"` を返します。  
> ✅ **受信解析（`parse_beacon`）は有効**です。
...
```lua
-- ❌ 送信: 非対応（nil + エラーを返す）
-- ble.beacon_start({ msg="Hello!", type="broadcast" })

-- ✅ 受信: スキャンコールバック内で parse_beacon を呼ぶ
ble.on("scan", function(dev)
...
**iGate（BLE → インターネット中継）スクリプト例**:
```lua
-- WiFi 接続 LUNA がビーコンを受信して公開 LUNA サーバーへ転送する
ble.on("scan", function(dev)
  local b = ble.parse_beacon(dev.data or "")
