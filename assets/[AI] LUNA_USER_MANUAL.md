...
**ブランド名**: LUNA / LUNA Observatory / LUNA INDI Edition / **LUNA Beacon Edition**（AiBridgeMCP v3.9.1〜v8.9.4）
**対象**: RS-232C機器・Alpaca 対応機器・INDI 対応天文機器・天文撮影ソフト・BLE 地域ネットワークを活用したいユーザー・技術者
**リビジョン**: r3.1
**最終更新**: 2026-04-28
**作成日**: 2026-03-19
...
| リビジョン | 日付 | 変更内容 |
|-----------|------|---------|
| r3.1 | 2026-04-28 | v8.9.4 対応：fs_write / fs_append の書き込み上限を 1KB → 4KB に更新 |
| r3.0 | 2026-04-26 | v8.9.3 Beacon Edition 対応：セクション6「LUNA Beacon Network」新設（BLE Long Range・ビーコンプロトコル・iGate・AI コミュニティ参加）|
...
| `fs_write` | ファイル書き込み（最大 4KB・上書き） | `path`、`content`（必須） |
| `fs_append` | ファイル追記（最大 4KB） | `path`、`content`（必須） |
