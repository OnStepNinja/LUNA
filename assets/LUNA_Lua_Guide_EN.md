...
② Claude generates a Lua script
      ↓
③ Execute with lua_exec or lua_load + lua_run
      ↓
...
| Argument | Type | Description |
|----------|------|-------------|
| name | string | Name of the next script to run (must be saved with `lua_load`) |
...
-- Saved with lua_load("obs_measure", ...)
log.load()
...
-- Saved with lua_load("obs_finish", ...)
log.load()
log.write("obs: sequence complete")
notify.set()   -- notify Claude of final completion
```

> ⚠️ **Note**: Scripts launched with `luna.run()` must be saved in advance with `lua_load`. If a non-existent script name is specified, an error is recorded in log and the chain terminates.

---

### 2-6. Standard Lua 5.1 Libraries
...
### 2-7. Numbers, Hex, and Bitwise Operations (Hardware Verified)
...
-- Notification
notify.set()                        -- notify Claude of completion
...
Claude:  lua_load("volt_check", ...) → saved
         Next time, just say "run volt_check"
...
| `lua_load` | Save a script to SPIFFS with a name |
...
```
Claude operations:
  lua_load("temma_goto", <above code>)  ← change ra_target / dec_target first
  lua_run("temma_goto")
  lua_log
    → === temma_goto start ===
...
> 💡 **Using Claude**: Tell Claude "slew to M42" and it will look up the coordinates, calculate `ra_hours` / `dec_deg`, and execute this script with `lua_load` + `lua_run`.
...
temma_init → temma_goto → temma_confirm
              (automatic)   (automatic)
  ↓
notify.set() after completion → notification to Claude
```
...
```
1. Save the 3 scripts with lua_load:
   lua_load("temma_init",    <script ① above>)
   lua_load("temma_goto",    <script ② above>)
   lua_load("temma_confirm", <script ③ above>)
