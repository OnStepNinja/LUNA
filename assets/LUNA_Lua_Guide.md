...
② Claude が Lua スクリプトを生成
      ↓
③ lua_exec または lua_load + lua_run で実行
      ↓
④ lua_status で完了を確認
      ↓
⑤ lua_log で結果を確認
...
Claude に「スクリプトが完了した」ことを通知するフラグを立てます。

```lua
notify.set()
```

> **推奨**: 長時間動作するスクリプトの末尾には必ず記述してください。
> Claude がこのフラグを検知すると、自動的に `lua_log` を取得して結果を確認します。
...
| 引数 | 型 | 説明 |
|------|----|------|
| name | string | 次に実行するスクリプト名（`lua_load` で保存済みのもの） |
...
-- lua_load("obs_finish", ...) で保存
log.load()
log.write("obs: シーケンス完了")
notify.set()   -- Claude に最終完了を通知
```

> ⚠️ **注意**: `luna.run()` で起動するスクリプトは事前に `lua_load` で保存しておく必要があります。存在しないスクリプト名を指定した場合はエラーが log に記録されてチェーンが終了します。

---

### 2-6. 標準 Lua 5.1 ライブラリ
...
Claude   ：lua_load("volt_check", ...) → 保存完了
...
| `lua_load` | スクリプトを名前付きで SPIFFS に保存 |
...
```
Claude の操作:
  lua_load("temma_goto", <上記コード>)  ← ra_target / dec_target を書き換えてから
  lua_run("temma_goto")
  lua_log
    → === temma_goto start ===
...
> 💡 **Claude に任せる場合**: 「M42 に導入して」と伝えると Claude が座標を調べて `ra_hours` / `dec_deg` を計算し、このスクリプトを `lua_load` + `lua_run` で実行します。
...
log.save()
notify.set()                       -- Claude に完了通知
```

#### 使い方

```
1. 3本のスクリプトを lua_load で保存:
   lua_load("temma_init",    <上記コード①>)
   lua_load("temma_goto",    <上記コード②>)
   lua_load("temma_confirm", <上記コード③>)
