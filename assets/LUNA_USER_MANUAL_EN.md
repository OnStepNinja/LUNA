...
**Brand Name**: LUNA / LUNA Observatory (AiBridgeMCP v3.9.1 / v5.9.1)
**Audience**: Users and engineers who want to control RS-232C and Alpaca-compatible devices with AI
**Revision**: r1.5
...
| **Persist scripts** | `lua_save` saves scripts to SPIFFS — survives power loss |
| **Autonomous script chaining** | `luna.run()` chains multiple scripts without Claude |
| **Async notification to Claude** | `notify.set("message")` lets scripts alert Claude from within |
| **Auto-delivery of usage guide** | On connection, `prompts/get luna_guide` runs automatically |
| **MCP Resources (v5.9.1)** | Claude reads device state naturally, like reading a gauge |
| **http module (v5.9.1)** | Lua scripts directly control Alpaca/HTTP devices over LAN |
...
### Supported Hardware (Binary Selection)

LUNA firmware is provided as two separate binaries — choose the one that matches your board.

| Binary | Target Board | RS-232C Port |
|--------|-------------|--------------|
| `LUNA_v3.9.1_Standard.bin` | Generic ESP32 development board | Serial2 (GPIO16 / GPIO17) |
| `LUNA_v3.9.1_OnStepNinja_V2.bin` | OnStepNinja V2 board | Serial (UART0) |

Flash the binary that matches your hardware.
You can verify which binary is running by checking the `target_board` field in `get_network_status`.

```
Example: {"target_board": "Standard", "serial_baud": 9600, ...}
```
...
| Item | Details |
|------|---------|
| **Parameters** | `name` (required): Script name saved with `lua_save` |
| **Returns** | `{"status":"started"}` (asynchronous — returns immediately) |
...
Checks the notification flag and message set by `notify.set()` in a Lua script.

| Item | Details |
|------|---------|
| **Parameters** | None |
| **Returns (no notification)** | `{"flag":0, "uptime":..., "heap":...}` |
| **Returns (notification present)** | `{"flag":1, "message":"your message", "uptime":..., "heap":...}` |

**Usage**: When a Lua script calls `notify.set("message string")`, Claude can detect it with `check_notify` and receive both the flag and the message. Both are automatically cleared after being read.

**How to check notifications (Claude Desktop)**: After running a script, simply tell Claude "check notify" and it will automatically call `check_notify` for you.
...
### 5.6 Resources Tools (New in v5.9.1)

> **📖 Details**: See `LUNA_Observatory_Resources_Guide_EN.md`.

MCP Resources are "gauges." They let Claude naturally read real-time device state — telescope position, temperature, switch status — during conversation, without explicit tool calls.

#### Difference Between Tools and Resources

| | Tools | Resources |
|--|-------|-----------|
| **Analogy** | Pressing a button | Reading a gauge |
| **Purpose** | Send commands, take action | Read status, get data |
| **How Claude uses them** | Explicitly called | Referenced naturally in context |

**Example:**

```
[Tools only (before v5.9.1)]
User: "Where is the telescope pointing?"
Claude: calls serial_query(":GR#") → calls serial_query(":GD#") → answers
...
| Tool | Function |
|------|----------|
| `resources/list` | List all available Resources (Claude Desktop references this automatically) |
| `resources/read` | Read the current value of a Resource by URI |

#### How to Create a Resource (Lua Script)

Save a script named `lua_resource_{name}.lua` to SPIFFS — it is automatically published as a Resource.
...
```lua
-- lua_resource_mount.lua (telescope RA/Dec)
...
With LUNA Observatory connected, Claude can reference Resources naturally:
...
[Notify Claude of completion or error from a Lua script]
  → call notify.set("message") in script → Claude polls with check_notify
...
[Make device state readable by Claude naturally (v5.9.1)]
  → save lua_resource_{name}.lua with lua_save → auto-published as luna://lua/{name}

[Check which Resources are currently available (v5.9.1)]
  → Ask Claude "check available resources" (runs resources/list)

[Control Alpaca/HTTP devices from Lua over LAN (v5.9.1)]
  → Use http.get() / http.put() inside a Lua script
```
...
② Claude generates, saves, and runs the Lua script
  lua_save("voltage_monitor", <script>) → saved to SPIFFS
  lua_run("voltage_monitor")            → execution starts

③ Claude can disconnect (ESP32 continues running on its own)

④ On error
  notify.set("Voltage anomaly: 0.8234V") fires
  Claude receives message via check_notify
...
#### http module (New in v5.9.1)

Sends HTTP requests to devices on the LAN. Used for integration with AiBridge (ASCOM/Alpaca).
...
- Blocking operation. Keep timeouts short (GET: 1000ms / PUT: 2000ms recommended)
- Always check for `""` return value on error or timeout

```lua
-- Get AiBridge switch state
local s0 = http.get("http://192.168.3.7/api/v1/switch/0/getswitch?Id=0", 1000)
if s0 == "" then return '{"error":"no response"}' end
return '{"port0":' .. s0 .. '}'

-- Turn on AiBridge switch
...
| **Lua Autonomous Control** | lua_exec / lua_save / lua_run / lua_status / lua_stop / lua_log / lua_list |
