...
**対象ファームウェア**: AiBridgeMCP v3.9.1 LUNA / v5.9.1 LUNA Observatory / v5.9.2 / v5.9.3 / v5.9.4 / v5.9.4a / v5.9.5 / v5.9.6 / v6.9.2 / v6.9.3 INDI Edition
**ドキュメントリビジョン**: r1.22
**最終更新**: 2026-04-15
...
- 第2章　Lua API リファレンス
  ...
  - 2-8. gpio モジュール — GPIO 直接制御（v5.9.3）
  ...
  - 2-17. 標準 Lua 5.1 ライブラリ
  - 2-18. 数値型・16進数・ビット演算（実機確認済み）
  - 2-19. 使用できない機能
  - 2-20. クイックリファレンス一覧
- 第3章　実用スクリプト集
  - 3-1. 機器の動作確認
...
#### HTML を SPIFFS に保存する方法（1KB 上限に注意）

`fs_write` / `fs_append` は **1KB（約994バイト）上限**です。  
HTML が大きい場合は分割書き込みを使います：

```
1. fs_write: path="/my_app.html", content=<HTMLの最初の部分（〜900B）>
2. fs_append: path="/my_app.html", content=<続き（〜900B）>
...
gpio.mode(pin, 0 or 1)         -- ピンモード設定（0=INPUT / 1=OUTPUT）
...
| `fs_write` | SPIFFS にファイルを書き込み（最大 1KB）（v5.9.x/v6.9.x） |
| `fs_append` | SPIFFS ファイルに追記（最大 1KB）（v5.9.x/v6.9.x） |
